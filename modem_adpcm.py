...
DISPLAY_MODEM_SETTINGS = "ATI12"
ENABLE_ECHO_COMMANDS = "ATE1"
ENABLE_FORMATTED_CID = "AT+VCID=1"
ENABLE_VERBOSE_CODES = "ATV1"
ENTER_VOICE_MODE = "AT&H4+FCLASS=8"
ENTER_TELEPHONE_ANSWERING_DEVICE_MODE = "AT+VLS=2"  # DCE off-hook
SET_VOICE_TRANSMIT_VOLUME = "AT+VGT="
ENTER_VOICE_TRANSMIT_DATA_STATE = "AT+VTX"
END_VOICE_TRANSMIT_DATA_STATE = DLE_CODE + ETX_CODE
FACTORY_RESET = "ATZ2"
GO_OFF_HOOK = "ATH1"
GO_ON_HOOK = "ATH0"
SET_VOICE_COMPRESSION_METHOD = "AT+VSM=4,9600"  # 4 = 4-bit ADPCM, 9.6 kHz
TERMINATE_CALL = "ATH"
...
class Modem(object):
    ...
    def set_sound_volume(self, volume):
        """Set transmitted sound volume."""
        print("Set volume: " + str(volume))
        self._serial.cancel_read()
        self._lock.acquire()
        try:
            if not self._send(SET_VOICE_TRANSMIT_VOLUME + str(volume), "OK"):
                print("Error: Failed to set transmitted sound volume.")
        finally:
            self._lock.release()
    ...
    def _send(self, command, expected_response=None, response_timeout=10):
    ...
    def _read_response(self, expected_response, response_timeout_secs):
        ...
        try:
            while 1:
                modem_data = self._serial.readline()
                print(modem_data)
                response = modem_data.strip((' \t\n\r' + DLE_CODE).encode()).decode("utf-8")
    ...
    def open_serial_port(self):
        ...
        com_ports_list = [s.decode("utf-8") for s in com_ports.split(b'\n')]
    ...
    def _init_serial_port(self, com_port):
        ...
        self._serial.baudrate = 115200  # 9600
...
def textToWav(text,file):
    print(text)
    subprocess.call(["espeak", "-vru", "-w " + file, "-s 85", text])
...
def main(argv):
    ...
    modem._send(ENTER_VOICE_MODE, "OK")
    modem.set_sound_volume(250)
    modem._send("ATDT5524085", "OK")
    time.sleep(20)
    modem.play_audio_as_adpcm("../message.wav")
    modem._send(TERMINATE_CALL, "OK")
    modem._send(FACTORY_RESET, "OK")
